...
int lenboard::measure(){
    ...
    if(pointer > 10){
        ...
        if(!(measurement[0] == 0xDE && measurement[1] == 0xAD && measurement[2] == 0xBE && measurement[3] == 0xEF))
            return -2;
        if(samplerate != (measurement[4]*16777216 + measurement[5]*65536 + measurement[6]*256 + measurement[7]))
            return -2;
...
int lenboard::getvalue(int count, int channel){
    ...
    int offset = count*((activechannels[0] ? 1 : 0)+(activechannels[1] ? 1 : 0)+(activechannels[2] ? 1 : 0)+(activechannels[3] ? 1 : 0))+chnum;
...
int portlist::refresh(){
    ...
    while((pentry = readdir(pdir))){
        if((pentry->d_name)[0] == 'c' && (pentry->d_name)[1] == 'u' && (pentry->d_name)[2] == '.'){
